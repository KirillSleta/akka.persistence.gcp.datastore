...
{
    /// <summary>
    /// Some integration tests for Journal and Snapshot plugins.
    /// </summary>
    ...
    //   public DatastoreIntegrationSpec(ITestOutputHelper output)
    //       : base(TestSetupHelpers.Config, "DatastoreIntegration", output: output)
    //   {
    ...
    //   [Fact]
    //   public void Datastore_journal_should_restore_state_after_restart()
    //   {
    //       var actor1 = Sys.ActorOf(Props.Create<PersistentActorA>(_actorId),_actorId);
    //       actor1.Tell(PoisonPill.Instance);
    //   }

    //   [Fact]
    //   public void Datastore_journal_should_write_and_replay_messages()
    //   {
    //       // Start a persistence actor and write some messages to it
    //       var actor1 = Sys.ActorOf(Props.Create<PersistentActorA>(_actorId));
    //       actor1.Tell(new DeleteToCommand(16L, true));
    ...
    //   [InlineData(true)]
    //   [InlineData(false)]
    //   public void Datastore_journal_should_not_replay_deleted_messages(bool permanentDelete)
    //   {
    //       // Listen for delete messages on the event stream
    //       var receiverProbe = CreateTestProbe();
    //       Sys.EventStream.Subscribe(receiverProbe.Ref, typeof(DeleteMessagesTo));
    //       //Sys.EventStream.Subscribe(receiverProbe.Ref, typeof(HandledMessage));
    ...
    //       receiverProbe.ExpectMsg<DeleteMessagesTo>();
    ...
    //   /// a Handled response.
    //   /// </summary>
    //   private void WriteAndVerifyMessages(IActorRef persistentActor, long start, long end, bool isRecovering = false)
    //   {
    //       long lastSeqNr = 0;
    ...
    //           if (i == start)
    //           {
    //               object ack = ReceiveOne();
    //               var handledMsg = Assert.IsType<HandledMessage>(ack);
    //               lastSeqNr = handledMsg.SequenceNumber -1;
    //            }
    //            else
    //           ExpectHandled(msg, i + lastSeqNr, isRecovering);
    //       }
    //   }
